...
> This directory is a **standalone SwiftPM package**. It is deliberately not a target of
> the root `oci-swift-sdk` package, and it lives outside `Sources/` and `Tests/`, so
> SwiftPM at the repo root never scans it: **swift-otel never enters the SDK's dependency
> graph**, and `swift build` / `swift test` at the repo root ignore this directory
> entirely. It depends on `oci-swift-sdk` from GitHub so the function container can be
> built without the surrounding repo in its Docker context. Nothing here runs in CI; it
> must be built and run by hand, and it needs a live APM domain.
...
The `Dockerfile` here builds `apm-trace-function` the same way
[`Tests/functions-live-test`](../../Tests/functions-live-test/README.md) builds its
function (multi-stage `swift:6.2` → `swift:6.2-slim`, non-root `fn` user at uid/gid 1000).
Follow that README for the OCIR push, application/function creation and IAM steps; the
only addition is enabling tracing on the application and the function, and giving the
function egress to the APM data-upload endpoint. Nothing else is needed: no function
configuration, no dynamic group, no policy — APM ingestion is data-key-authenticated.

---

## Caveats

Ingestion facts from `OBSERVABILITY.md` §2.3, plus what this example ran into.
