...
final class OCIKitTests: XCTestCase {
    ...
    func test_if_list_compartments_returns_valid_string() async throws {
        let signer = try APIKeySigner(configFilePath: ociConfigFilePath, configName: ociProfileName)
        let identityClient = try IdentityClient(region: .fra, signer: signer)
        let listCompartments = try await identityClient.listCompartments(compartmentId: "ocid1.tenancy.oc1..aaaaaaaapt3esrvwldrfekea5ucasigr2nof7tjx6ysyb4oo3yiqgx2d72ha")

        XCTAssertFalse(listCompartments.isEmpty, "Namespace should not be empty")
    }
